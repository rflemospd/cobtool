...
async function neGeneratePDF(){
  ...
  function drawReplaceText(item, viewport, text, size, isBold, pageTarget){
    ...
    if(String(text||'').trim()){
      ...
      target.drawText(text, {x: rect.x, y: textY, size: fontSize, font: drawFont, color: rgb(0,0,0)});
    }
    return true;
  }
  ...
  if(razaoItem){
    razaoPlaced=drawReplaceText(razaoItem, viewport, razao, 12, true, page1);
  }
  if(cnpjItem && cnpj){
    cnpjPlaced=drawReplaceText(cnpjItem, viewport, cnpjText, 12, true, page1);
  }
  ...
  if(!razaoPlaced || (cnpj && !cnpjPlaced)){
    ...
    if(anchorItem){
      ...
      if(!razaoPlaced){
        razaoPlaced=drawTextAt(anchorRect.x, anchorRect.yTop + lineGap * 2 + lift, fontSize, razao, true, page1);
      }
      if(cnpj && !cnpjPlaced){
